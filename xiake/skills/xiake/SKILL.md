---
name: xiake
description: This skill should be used when the user asks to 玩游戏, 玩侠客擂台, 闯关, 招募侠客, 抽卡, 来一发, 急速补充, 查看额度, AI对战, 擂台, 组队, 换人, 查伤, 疗伤, 装技能珠, 查看侠客, 查看关卡, 战绩, 门派, lore, or otherwise interact with 侠客擂台 — a Claude Code-native 武侠 3v3 回合制对战链游 with 7 大门派, 12 关剧情, PVP 擂台, and 抽卡经济 (free + paid gacha, BOSS rewards, 七派相克).
...
```bash
# Windows
setx XIAKE_CLI_PATH "<repo>/skill/dist/cli.js"
# macOS / Linux / WSL
export XIAKE_CLI_PATH="<repo>/skill/dist/cli.js"
```

`<repo>` 是 `https://github.com/PAtrickpp33/Xiake-onchain` clone 到本地的路径。未设置时,请提示玩家设置该变量(指向 `skill/dist/cli.js`),再继续。调用模板:`node "$XIAKE_CLI_PATH" <command>`。
